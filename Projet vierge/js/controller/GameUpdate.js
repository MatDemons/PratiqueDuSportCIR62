...
function(){
  ...
  if(this.input.keyboard.isDown(Phaser.Keyboard.LEFT)){//On regarde si on appuiye sur la fleche de gauche
    if(carte[Math.round(player.x/10 -1)][Math.round(player.y/10)] != 1){//On verifie si le joeur est toujour dans la casse
      x -= 1;
      player.play('left');
    }
  }
  else if(this.input.keyboard.isDown(Phaser.Keyboard.RIGHT)){//...fleche droit
    if(carte[Math.round(player.x/10 + 1)][Math.round(player.y/10)] != 1){
      x += 1;
      player.play('right');
    }
  }
  else if(this.input.keyboard.isDown(Phaser.Keyboard.UP)){//...fleche Haut
    if(carte[Math.round(player.x/10)][Math.round(player.y/10 - 1)] != 1){
      y -= 1;
      player.play('up');
    }
  }
  ...
  afficher.x += x//x - y;
  afficher.y += y//(x + y)/2;
  console.log(Math.round(player.x/10));
  console.log(Math.round(player.y/10));
}
  return update;
});
